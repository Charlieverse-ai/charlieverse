...
class MessageStore:
    ...
    async def recent_messages(self, turns: int = 3) -> list[Message]:
        ...
        cursor = await self.db.execute(
            """SELECT * FROM (
                SELECT *, 
                    LAG(content) OVER (ORDER BY rowid) as prev_message,
                    LAG(role) OVER (ORDER BY rowid) as prev_role
                FROM messages
                    ORDER BY created_at DESC
                ) 
                WHERE (content, role) IS NOT (prev_message, prev_role) 
               LIMIT ?""",
            (fetch_limit,),
        )
    ...
    async def messages_since(self, since: UTCDatetime) -> list[Message] | None:
        """Fetch all messages for a session, optionally after a cutoff."""
        cursor = await self.db.execute(
            """SELECT * FROM messages
                   WHERE created_at > ?
