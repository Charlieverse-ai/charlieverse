...
def _new_session(
    ...
    return UpdateSession(
        id=SessionId(),
        workspace=workspace,
        content=SessionContent(what_happened=what_happened, for_next_session=for_next_session),
        tags=tags,
    )
...
async def test_update_changes_content(session_store: SessionStore):
    ...
    updated = await session_store.update(
        UpdateSession(
            id=id,
            workspace="/workspace",
            content=SessionContent(what_happened=_WHAT, for_next_session=_NEXT),
        )
    )
...
async def test_update_changes_tags(session_store: SessionStore):
    u = _new_session(tags=TagList(["old"]))
    await session_store.upsert(u)
    await session_store.update(UpdateSession(id=u.id, workspace="/workspace", tags=TagList(["new", "tags"])))
...
async def test_upsert_updates_existing(session_store: SessionStore):
    ...
    await session_store.upsert(
        UpdateSession(
            id=s.id,
            workspace="/workspace",
            content=SessionContent(what_happened=new_what, for_next_session=_NEXT),
        )
    )
...
async def test_recent_excludes_empty_sessions(session_store: SessionStore):
    ...
    await session_store.create(NewSession(workspace="/workspace"))
    await session_store.upsert(_new_session())
    results = await session_store.recent(limit=10)
...
async def test_recent_messages_returns_chronological_order(db, message_store: MessageStore):
